...
public class WaveCmd extends Command {
  ...
  public void initialize() {
    ...
    // actually start the wave until that point. If it needs to be restarted, then manually
    // rotate the robot to point forward (direction of parade) before re-starting the wave command.
    m_swerveDrive.zeroGyro();
	
    m_startTime = System.currentTimeMillis();
    m_waitTime = 2000;   // initial pause gives time for master arm to move up
	
	  m_noteConductor.startWavingAtCrowd(IAC.NORMAL_WAVE_SPEED, IAC.NORMAL_WAVE_MAGNITUDE);
  }
  ...
  public void execute() {
    ...
    // Initializing m_waitTime to 2000 gives the master arm time to get into waving position
    // before rotation starts.
    // Note that after initialization, the inner arm wave speed is increased, and the
    // magnitude of the wave simultaneously decreased and centered about the horizontal position,
    // for the duration of each rotation pause.
    // After each pause expiration, slow down the wave, and make it more vertical (oscillate
    // about 50 degrees instead of about 0).
    if (m_waitTime > 0) {
    ...
    if (((m_waveRotationDirection == 1.0) && (m_currentHeading > SDC.WAVE_ROTATION_EXTENT))
      ...
      m_noteConductor.startWavingAtCrowd(IAC.FAST_WAVE_SPEED, IAC.FAST_WAVE_MAGNITUDE);
